...
use WebEd\Modules\Relations\Http\DataTables\CategoriesListDataTable;
use WebEd\Modules\Relations\Http\Requests\CreateCategoryRequest;
use WebEd\Modules\Relations\Http\Requests\UpdateCategoryRequest;
...
class CategoryController extends BaseAdminController
{
    ...
    public function __construct(CategoryRepositoryContract $repository)
    {
        ...
        $this->middleware(function (Request $request, $next) {
            ...
            $this->breadcrumbs
                ...
                ->addLink(trans('relations::base.categories.page_title'), route('admin::blog.categories.index.get'));
    ...
    {
        $this->setPageTitle(trans('relations::base.categories.page_title'));

        $this->dis['dataTable'] = $dataTables->run();

        return do_filter(BASE_FILTER_CONTROLLER, $this, WEBED_RELATIONS_CATEGORIES, 'index.get', $dataTables)->viewAdmin('categories.index');
    }
    ...
    public function postListing(CategoriesListDataTable $dataTables)
    {
        ...
        return do_filter(BASE_FILTER_CONTROLLER, $data, WEBED_RELATIONS_CATEGORIES, 'index.post', $this);
    }
        ...
        if ($this->request->get('customActionType', null) === 'group_action') {
            ...
            switch ($actionValue) {
                case 'deleted':
                     ...
                     * Delete items
                     */
                    $ids = do_filter(BASE_FILTER_BEFORE_DELETE, $ids, WEBED_RELATIONS_CATEGORIES);
                    $result = $this->repository->deleteCategory($ids);
                    do_action(BASE_ACTION_AFTER_DELETE, WEBED_RELATIONS_CATEGORIES, $ids, $result);
                    break;
                case 'activated':
                case 'disabled':
    ...
    {
        do_action(BASE_ACTION_BEFORE_CREATE, WEBED_RELATIONS_CATEGORIES, 'create.get');
        ...
        $this->setPageTitle(trans('relations::base.categories.form.create_category'));
        $this->breadcrumbs->addLink(trans('relations::base.categories.form.create_category'));

        return do_filter(BASE_FILTER_CONTROLLER, $this, WEBED_RELATIONS_CATEGORIES, 'create.get')->viewAdmin('categories.create');
    }

    public function postCreate(CreateCategoryRequest $request)
    {
        do_action(BASE_ACTION_BEFORE_CREATE, WEBED_RELATIONS_CATEGORIES, 'create.post');
        ...
        do_action(BASE_ACTION_AFTER_CREATE, WEBED_RELATIONS_CATEGORIES, $result);
    ...
    public function getEdit($id)
    {
        ...
        $item = do_filter(BASE_FILTER_BEFORE_UPDATE, $item, WEBED_RELATIONS_CATEGORIES, 'edit.get');
        ...
        $this->setPageTitle(trans('relations::base.categories.edit_item') . ' #' . $item->id);
        $this->breadcrumbs->addLink(trans('relations::base.categories.edit_item'));

        $this->dis['object'] = $item;

        return do_filter(BASE_FILTER_CONTROLLER, $this, WEBED_RELATIONS_CATEGORIES, 'edit.get', $id)->viewAdmin('categories.edit');
    }
    ...
    public function postEdit(UpdateCategoryRequest $request, $id)
    {
        ...
        $item = do_filter(BASE_FILTER_BEFORE_UPDATE, $item, WEBED_RELATIONS_CATEGORIES, 'edit.post');

        $result = $this->repository->updateCategory($item, $data);

        do_action(BASE_ACTION_AFTER_UPDATE, WEBED_RELATIONS_CATEGORIES, $id, $result);
    ...
    {
        $id = do_filter(BASE_FILTER_BEFORE_DELETE, $id, WEBED_RELATIONS_CATEGORIES);

        $result = $this->repository->deleteCategory($id);

        do_action(BASE_ACTION_AFTER_DELETE, WEBED_RELATIONS_CATEGORIES, $id, $result);
