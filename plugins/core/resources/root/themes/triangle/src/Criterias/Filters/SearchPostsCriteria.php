...
class SearchPostsCriteria extends AbstractCriteria
{
    ...
    {
        return $model
            ->leftJoin('posts_tags', 'relations.id', '=', 'posts_tags.post_id')
            ->leftJoin('blog_tags', 'blog_tags.id', '=', 'posts_tags.tag_id')
            ->where('relations.status', '=', 'activated')
            ->where(function ($query) {
                return $query
                    ->orWhere('relations.title', 'LIKE', '%' . $this->keyword . '%')
                    ->orWhere('relations.slug', 'LIKE', '%' . $this->keyword . '%')
